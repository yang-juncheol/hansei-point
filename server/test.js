  ...
  try {
    const itemTemplates = [
      ...
      },
      {
        product_name: "녹차",
        product_image_url:
          "https://health.chosun.com/site/data/img_dir/2023/06/16/2023061602342_0.jpg",
        product_price: 2000,
      },
      {
        product_name: "아리피규어",
        product_image_url:
          "https://img.danawa.com/prod_img/500000/791/519/img/15519791_1.jpg?_v=20211026132708",
        product_price: 30000,
      },
      {
        product_name: "신상혁",
        product_image_url:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSL9rkCP9JN_N9lwkcmbtQJWOAiY2jnrzyiTWDK8itnKNxnyNkegQ0vZX9VHm994IvhlJQ&usqp=CAU",
        product_price: 999999999,
      },
      {
        product_name: "베라아이스크림",
        product_image_url:
          "https://www.spcmagazine.com/wp-content/uploads/2019/07/%EC%9D%B4%EB%AF%B8%EC%A7%80-%EB%B0%B0%EC%8A%A4%ED%82%A8%EB%9D%BC%EB%B9%88%EC%8A%A4_-%EC%9D%B8%EA%B8%B0-%ED%94%8C%EB%A0%88%EC%9D%B4%EB%B2%84-2%EC%A2%85-%EC%9E%AC%EC%B6%9C%EC%8B%9C.jpg",
        product_price: 5000,
      },
    ];
    for (var i = 0; i < 100; i++) {
  ...
  try {
    if (req.method === "GET") {
      if (/\/api\/hansei\/member\/([0-9]{1,})\/point/.test(pathname)) {
        const memberId = +pathname.replace(
          /\/api\/hansei\/member\/([0-9]{1,})\/point/,
          (p0, p1) => p1
        );
        ...
        const memberId = +pathname.replace(
          /\/api\/hansei\/member\/([0-9]{1,})\/orders/,
          (p0, p1) => p1
        );
        const datas = await db.select(
          "my_product",
          (data) => data.memberIdId === memberId
        );
        const result = [];
        for (var i = 0; i < datas.length; i++) {
          var product = await db.selectOne(
            "product",
            (data) => data.product_id === datas[i].product_id
          );
          if (product) {
            result.push({
        ...
        req.on("end", () => {
          try {
            const postdata = JSON.parse(
              Buffer.from(Buffer.concat(chunks), "utf8").toString()
            );
            resolve(postdata);
          } catch (e) {
            reject(e);
      ...
      if (pathname.startsWith("/api/hansei/login")) {
        ...
        const phoneNumber = pathname.replace(
          /\/api\/hansei\/user\/([0-9]{1,})\/point/,
          (p0, p1) => p1
        );
        ...
        const memberId = +pathname.replace(
          /\/api\/hansei\/user\/([0-9]{1,})\/order/,
          (p0, p1) => p1
        );
        const userData = await db.selectOne(
          "member",
          (data) => data.memberId === memberId
        );
        const productData = await db.selectOne(
          "product",
          (data) => data.product_id === postdata.productId
        );
        ...
        const datas = await db.select(
          "member",
          (data) => data.phoneNumber === postdata.phoneNumber
        );
        if (datas.length !== 0) {
          res.writeHead(404, commonHeader);
          return res.end(
      ...
      if (
        /\/api\/hansei\/member\/([0-9]{1,})\/order\/([0-9]{1,})/.test(pathname)
      ) {
        let memberId = 0,
          order_id = 0;
        pathname.replace(
          /\/api\/hansei\/member\/([0-9]{1,})\/order\/([0-9]{1,})/,
          (p0, p1, p2) => {
        ...
        const orderData = await db.selectOne(
          "my_product",
          (data) => data.idx === order_id
        );
        if (!orderData) {
          res.writeHead(404, commonHeader);
          return res.end(
        ...
        const productData = await db.selectOne(
          "product",
          (data) => data.product_id === orderData.product_id
        );
